class UPComBotCommand():#命令生成
        
    def Command(self, angle=0, speed=0, turn=0, time=500):#角度，速度，转向，时间；返回转向命令        
        data = [0]*8
        data[0] = angle&0xFF#保留低八位
        data[1] = (angle>>8)&0xFF#保留高八位
        data[2] = speed&0xFF
        data[3] = (speed>>8)&0xFF
        data[4] = turn&0xFF
        data[5] = (turn>>8)&0xFF
    ...
    def call_action_by_name(self, name):
        length = len(name.encode())#将命令名以encode形式编码并测量长度
        # data = name.encode()
        data = [0] * 5
        data[0] = 4 & 0xFF
        data[1] = (3000 >> 8) & 0xFF
        data[2] = (3000 % 0x100) & 0xFF
        data[3] = (500 >> 8) & 0xFF
        data[4] = (500 % 0x100) & 0xFF
        buffer, length = self.GenerateCmd(0x07, 0x5c, 0x05, data)
        return buffer

    def GenerateCmd(self, device, cmd, len, data):#用驱动，命令，长度，数据生成命令
        buffer = [0]*(len+6)
        buffer[0] = 0xF5
        buffer[1] = 0x5F
    ...
    def check_operation(self, data):#命令正确性检查
        l = len(data)
        check = data[2]
        check = check+data[3]
        check = check+data[4]
